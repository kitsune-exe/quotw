...
fn draw_table(frame: &mut Frame, area: Rect, state: &AppState, theme: &ThemeColors) {
    ...
    let table = Table::new(rows, widths)
        .header(header)
        .block(block)
        .row_highlight_style(Style::new().bg(theme.selected).fg(theme.bg).add_modifier(Modifier::BOLD))
        .column_spacing(1);
...
fn draw_title(frame: &mut Frame, area: Rect, state: &AppState, theme: &ThemeColors) {
    ...
    let title_text = if index.is_valid() {
        ...
        Line::from(vec![
            Span::styled("📈 台灣股市即時行情  ", Style::new().fg(theme.header_bg).bg(theme.header_fg).add_modifier(Modifier::BOLD)),
            Span::styled(index.name.clone(), Style::new().fg(theme.header_bg).bg(theme.header_fg).add_modifier(Modifier::BOLD)),
            Span::styled(" ", Style::new().fg(theme.header_bg).bg(theme.header_fg)),
            Span::styled(index.price_display(), Style::new().fg(price_color).bg(theme.header_fg).add_modifier(Modifier::BOLD)),
            Span::styled(" ", Style::new().fg(theme.header_bg).bg(theme.header_fg)),
            Span::styled(index.change_display(), Style::new().fg(change_color).bg(theme.header_fg).add_modifier(Modifier::BOLD)),
            Span::styled(" ", Style::new().fg(theme.header_bg).bg(theme.header_fg)),
            Span::styled(index.pct_display(), Style::new().fg(change_color).bg(theme.header_fg).add_modifier(Modifier::BOLD)),
        ])
    } else {
        Line::from(vec![
            Span::styled("📈 台灣股市即時行情  大盤指數 --", Style::new().fg(theme.header_bg).bg(theme.header_fg).add_modifier(Modifier::BOLD)),
        ])
    };
...
fn quote_to_row<'a>(q: &'a Quote, theme: &'a ThemeColors) -> Row<'a> {
    ...
    let (price_color, change_color, row_style) = if !q.is_valid() {
        ...
        // 漲停：紅底白字
        (Color::White, Color::White, Style::new().fg(Color::White).bg(theme.up))
    } else if is_limit_down {
        // 跌停：綠底白字
        (Color::White, Color::White, Style::new().fg(Color::White).bg(theme.down))
    ...
    Row::new(vec![
        ...
        Cell::from(q.price_display()).style(Style::new().fg(price_color).add_modifier(Modifier::BOLD).bg(row_style.bg.unwrap_or(theme.bg))),
        Cell::from(q.change_display()).style(Style::new().fg(change_color).bg(row_style.bg.unwrap_or(theme.bg))),
        Cell::from(q.pct_display()).style(Style::new().fg(change_color).bg(row_style.bg.unwrap_or(theme.bg))),
        Cell::from(q.volume_display()).style(Style::new().fg(theme.fg).bg(row_style.bg.unwrap_or(theme.bg))),
    ])
    .height(1)
    .style(row_style)
}
...
fn draw_status(frame: &mut Frame, area: Rect, state: &AppState, theme: &ThemeColors) {
    ...
    let status = Paragraph::new(vec![
        ...
        Line::from(vec![
            Span::styled(" 導航: ", Style::new().fg(theme.header_bg).bg(theme.header_fg).add_modifier(Modifier::BOLD)),
        ...
        Line::from(vec![
            Span::styled(" 操作: ", Style::new().fg(theme.header_bg).bg(theme.header_fg).add_modifier(Modifier::BOLD)),
            Span::styled("a ", Style::new().fg(theme.selected).add_modifier(Modifier::BOLD)),
            Span::styled("新增股票  ", Style::new().fg(theme.fg)),
            Span::styled("A ", Style::new().fg(theme.selected).add_modifier(Modifier::BOLD)),
            Span::styled("新增分組  ", Style::new().fg(theme.fg)),
            Span::styled("e ", Style::new().fg(theme.selected).add_modifier(Modifier::BOLD)),
            Span::styled("編輯  ", Style::new().fg(theme.fg)),
            Span::styled("d ", Style::new().fg(theme.selected).add_modifier(Modifier::BOLD)),
            Span::styled("刪除", Style::new().fg(theme.fg)),
        ]),
        Line::from(vec![
            Span::styled(" 系統: ", Style::new().fg(theme.header_bg).bg(theme.header_fg).add_modifier(Modifier::BOLD)),
...
fn draw_popup(frame: &mut Frame, area: Rect, state: &AppState, theme: &ThemeColors) {
    ...
    match &state.popup {
        PopupState::AddStock { group, code, name, field: _, error } => {
        ...
        PopupState::EditStock { group, index: _, code, name, field } => {
...
fn draw_add_stock_popup(frame: &mut Frame, area: Rect, theme: &ThemeColors, group: &str, code: &str, _name: &str, _field: usize, error: &Option<String>) {
    let block = Block::default()
    ...
    let code_para = Paragraph::new(code_text).style(Style::new().fg(theme.selected).add_modifier(Modifier::BOLD));
...
fn draw_add_group_popup(frame: &mut Frame, area: Rect, theme: &ThemeColors, name: &str) {
    ...
    let name_para = Paragraph::new(name_text).style(Style::new().fg(theme.selected).add_modifier(Modifier::BOLD));
...
fn draw_edit_stock_popup(frame: &mut Frame, area: Rect, theme: &ThemeColors, group: &str, code: &str, name: &str, field: usize) {
    let block = Block::default()
    ...
    // Code input
    let code_label = if field == 0 { "► 代碼: " } else { "  代碼: " };
    ...
    // Name input
    let name_label = if field == 1 { "► 名稱: " } else { "  名稱: " };
